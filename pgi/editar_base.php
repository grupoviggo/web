...
<div class="content">
    <br>
<div class="panel">
    <h2><i class="fa-solid fa-user-pen"></i> Editar Base</h2>
</div>
<br>
<div class="panel2">
    ...
    if (isset($_GET['id_base'])) {
        ...
        $query = "SELECT * FROM bases WHERE id_base = $id_base";
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        $NOME = mysqli_real_escape_string($connect, $_POST['CONSULTOR_BASE_NOME']);


        // Atualiza os dados do usuário no banco de dados
        $update_query = "
            UPDATE bases
            SET CONSULTOR_BASE_NOME = '$NOME' WHERE id_base = $id_base
        ";
...
?>
        ...
        <form method="post">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="CONSULTOR_BASE_NOME" name="CONSULTOR_BASE_NOME"
                    value="<?php echo htmlspecialchars($user['CONSULTOR_BASE_NOME']); ?>" maxlength="150"
                    style="text-transform: uppercase;" oninput="this.value = this.value.replace(/[^A-Za-z\s]/g, '').toUpperCase()" required>
            </div>
            <!-- Campo Cargo com Select -->
            <br>
            <button type="submit" class="btn btn-primary">Salvar Alterações</button>
            <a href="Bases" class="btn btn-secondary">Cancelar</a>
        </form>
    </div>
</div>
