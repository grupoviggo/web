...
$query = "SELECT v.codigo_venda, v.tipo_cliente, v.cpf_vendedor, v.valor_venda, v.data_input, v.horario_venda, 
                 ...
                 u.nome AS backoffice_nome, dp.BACKOFFICE, dp.NOME_CLIENTE, dp.DOC_CLIENTE, dp.OFERTA, dp.STATUS_VENDA
          FROM vendas v
          ...
          WHERE v.status_venda NOT IN ('ATIVO', 'PENDENTE', 'CANCELADO')";
...
require 'consulta_status_fila.php';
...
<head>
    ...
    <style>
        ...
        /* Estilo para o tooltip (texto completo) */
        .tooltips {
            display: none;
            /* Oculto por padrão */
            position: absolute;
            ...
            z-index: 10;
            top: 30%;
            /* Posiciona abaixo do texto truncado */
            left: 0;
        ...
        td#no-right-border:hover .tooltips {
            display: block;
        }
    </style>
        ...
        <div class="content">
            <div class="panel titulo">
                <div class="header-container_2">
                    <h2 class="subpanel_h2"><i class="fa-solid fa-pen-to-square fa-xl"></i> Fila de Auditoria</h2>
                    <!-- Status auditoria -->
                            ...
                            while ($row = mysqli_fetch_assoc($result)) {
                                ...
                                switch ($status_venda_final) {
                                    case 'FINALIZADA':
                                        ...
                                        break;
                                    case 'EM FILA':
                                        $btn_class = 'btn btn-silver btn-sm'; // Cinza
                                        break;
                                    case 'REENTRADA':
                                        $btn_class = 'btn btn-pastel btn-sm'; // Cinza
                                        break;
                                    case 'AG. RETORNO':
                                        $btn_class = 'btn btn-salmao btn-sm'; // Azul
                                        break;
                                    case 'CANCELADO':
                                ...
                                echo "<td class='no-left-border-left' onclick='copiarTexto(this)' style='position: relative;'>"
                                    . htmlspecialchars(substr($codigo_venda, 0, 10))
                                    . "<span class='tooltip'>Copiado!</span></td>";
                                echo "<td id='no-right-border'>" . htmlspecialchars($row['data_input']) . "<br>"
                                    . htmlspecialchars(substr($row['horario_venda'], 0, 5)) . "</td>";
                                echo "<td id='no-right-border' style='font-size: 10px !important; text-align: center; vertical-align: middle;'>$nome_com_quebra</td>";
                                echo "<span class='tooltips'>" . $base_nome_resumido . "</span>";
                                echo "<td style='font-size: 10px !important; position: relative;' id='no-right-border'>";
                                echo "<span class='truncate'>" . $base_nome_resumido . "</span>";
                                echo "</td>";
                                ...
                                echo "<td id='no-right-border'>R$ " . htmlspecialchars($row['valor_venda']) . "</td>";
                                echo "<td id='no-right-border'>" . htmlspecialchars($row['PERFIL']) . "</td>";

                                echo "<td class='no-left-border-right'>
                <div style='display: flex; align-items: center; justify-content: space-between;'>
                    <a href='#' onclick='confirmarInativacao($codigo_venda)' class='same-size-btn $btn_class'>$status_venda_final</a>
                    <div class='dots-menu' style='position: relative;'>
                        <i class='fi fi-br-menu-dots-vertical fi-3xl tabular' style='color: #677074; cursor: pointer;' onclick='showMenu(this)'></i>
                        <div class='submenu_fila' id='submenu-container-$codigo_venda'></div>
                    </div>
                </div>
            </td>";
                                echo "</tr>";
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="tabularModalLabel" style="color: #000 !important;">Pendenciar Venda:
                            <span id="codigo_venda_display"><?php echo $codigo_venda; ?></span>
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button>
                    </div>
                    <div class="modal-body">
                        <form action="tabular_venda" method="POST" id="formTabulacao">
                            <input type="hidden" id="codigo_venda" name="codigo_venda"
                                value="<?php echo $codigo_venda; ?>">
                            <input type="hidden" id="backoffice" name="backoffice" value="<?php echo $backoffice; ?>">

                            <!-- Tabulação -->
                            <div class="mb-3">
                                <label for="tabulacao" class="form-label">Tabulação</label>
                                <select name="status_venda" id="tabulacao" class="form-select">
                                    <option value="">Selecione</option>
                                    <option value="REENTRADA">REENTRADA</option>
                                    <option value="FINALIZADA">FINALIZADA</option>
                                    <option value="AG_RETORNO">AG. RETORNO</option>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="sub_status" id="sub_status" class="form-select" disabled>
                                    <option value="">Selecione</option>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="descricao" id="descricao" class="form-select" disabled>
                                    <option value="">Selecione</option>
                                </select>
                            </div>
