...
if (isset($_GET['id_base'])) {
    ...
    // Atualiza o status da base para "INATIVO"
    $query = "UPDATE bases SET STATUS_BASE = 'INATIVO' WHERE id_base = $id_base";

    if (mysqli_query($connect, $query)) {
        $statusAtualizado = true; // Atualização bem-sucedida
    } else {
        echo "<p class='alert alert-danger'>Erro ao inativar base: " . mysqli_error($connect) . "</p>";
    }
} else {
    echo "<p class='alert alert-danger'>ID da base não foi especificado.</p>";
}
...
if (!empty($busca)) {
    $query .= " CONSULTOR_BASE_NOME LIKE '%" . mysqli_real_escape_string($connect, $busca) . "%'";
}
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="menu-item dropdown-rh">
            ...
            <div class="submenu" id="submenurh">
                <a href="painelrh_.php" class="page-link">Cadastrar Base</a>
            </div>
        </div>
       ...
       <div class="content">
            <div class="panel" style="padding: 1px;">
                <div class="header-container">
                    <h2>&nbsp;<i class="fa-solid fa-list-check"></i> Lista de Bases</h2>
                    <div class="button-container">
                        <form method="GET" class="d-flex align-items-center gap-10">
                            ...
                            <button type="submit" class="btn btn-primary search-btn" style="width: 160px;"><i class="fa-solid fa-magnifying-glass"></i> Buscar</button>
                            <button type="button" class="btn btn-secondary search-btn" style="width: 160px;" onclick="window.location.href='Bases.php'"><i class="fa-solid fa-eraser"></i> Limpar</button>
                        </form>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        if ($temResultados) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                echo "<tr>";
                                echo "<td id='no-left-border-middle'>" . htmlspecialchars($row['CONSULTOR_BASE_NOME']) . "</td>";
                                echo "<td id='no-left-border'>" . htmlspecialchars($row['STATUS_BASE']) . "</td>";
